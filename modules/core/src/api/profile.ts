...
import { AuthenticateSessionUtil, SignInUtil } from "@cicis/authentication";
import { AxiosHttpClient, AxiosHttpClientInstance } from "@cicis/http";
...
export const getUserInfo = (): Promise<ProfileInfoInterface> => {
    ...
    const requestConfig = {
        headers: HTTPRequestHeaders(ContextUtils.getRuntimeConfig().clientHost, AcceptHeaderValues.APP_SCIM, ContentTypeHeaderValues.APP_SCIM),
...
export const getProfileInfo = (onSCIMDisabled: () => void): Promise<ProfileInfoInterface> => {
    ...
    const requestConfig = {
        headers: HTTPRequestHeaders(ContextUtils.getRuntimeConfig().clientHost, AcceptHeaderValues.APP_SCIM,
...
export const getProfileSchemas = (): Promise<ProfileSchemaInterface[]> => {
    ...
    const requestConfig = {
        headers: HTTPRequestHeaders(ContextUtils.getRuntimeConfig().clientHost, AcceptHeaderValues.APP_SCIM, ContentTypeHeaderValues.APP_JSON),
        method: HttpMethods.GET,
    ...
    return httpClient.request(requestConfig)
        .then((response) => {
            ...
            console.log("attribute = " + response.data[0].attributes);
