...
export const UserSessionsWidget: FunctionComponent<{}> = (): JSX.Element => {
    ...
    const getUserSessions = (): void => {
        fetchUserSessions()
            .then((response) => {
                if (response && response.grants && response.grants.length && response.grants.length > 0) {
                    let sessions = [ ...response.grants ];

                    // Sort the array by last access time
                    sessions = _.reverse(_.sortBy(sessions, (session) => session.lastUsed));
