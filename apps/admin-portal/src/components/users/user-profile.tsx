...
import { Divider, Form, Grid, Icon, List, Placeholder, Popup, Responsive} from "semantic-ui-react";
import {deleteUser, updateProfileInfo} from "../../api";
...
import {EditSection} from "../../../../user-portal/src/components/shared";
...
export const UserProfile: FunctionComponent<ProfileProps> = (props: ProfileProps): JSX.Element => {
    ...
    const [ setUrlSchema ] = useState<ProfileSchema>();
                ...
                if(!schema.customAttribute){
                    if (schema.scimName.includes("urn:ietf:params:scim:schemas:extension:ibm:2.0:User")){
                        ...
                        if(schema.scimName === "phoneNumbers" && userInfo.phoneNumbers){
                            const localPhoneNumber = userInfo.phoneNumbers.find(phoneNumber => phoneNumber.type === schema.name);
                            if( localPhoneNumber){
                                tempProfileInfo.set(schema.name, localPhoneNumber.value);
                            }
                        } else if (schema.scimName === "emails" && userInfo.emails){

                            const localemail = userInfo.emails.find(email => email.type === "work");
                            if( localemail && schema.name === "mail"){
                                tempProfileInfo.set(schema.name, localemail.value);
                            }
                        } else if(schema.scimName === "groups"){
                            //tempProfileInfo.set(schema.name, userInfo.groups);
                        }else {
    ...
    const handleSubmit = (values: Map<string, string | string[]>, formName: string, schemaName: string): void => {
        ...
        if(schemaName === "mail"){
            user.emails[0].value = values.get(schemaName) as string;
        }if(schemaName === "mobile"){
            user.phoneNumbers[0].value = values.get(schemaName) as string;
        }else{
            _.set(user,formName,values.get(schemaName))
        }

        updateProfileInfo(user.id,user).then((response) => {
     ...
     */
    const generateProfileEditForm = (schema: ProfileSchema): JSX.Element => {
        if (editingForm && editingForm.size > 0 && editingForm.get(schema.name)) {
            const fieldName = schema.customAttribute ? schema.scimName : schema.name ;
            /*t("views:components.profile.fields." + schema.name.replace(".", "_"),
                ...
                <Grid padded={ true }>
                    <Grid.Row columns={ 3 }>
                        < Grid.Column mobile={ 6 } tablet={ 6 } computer={ 4 } className="first-column">
                            <List.Content>{ /*fieldName*/schema.customAttribute? schema.scimName : schema.name    }</List.Content>
                        </Grid.Column>
        ...
        /*
                const fieldName = t("views:components.user.profile.fields." +
                    schema.scimName.replace(".", "_"), { defaultValue: schema.displayName }
                );
                return (
                    <Grid.Row columns={ 1 }>
                        <Grid.Column mobile={ 12 } tablet={ 12 } computer={ 6 }>
                            <Field
                                ...
                                name={ schema.scimName }
                                label={ schema.customAttribute? schema.scimName : schema.name }
        ...
        <>
           {/* {
                !_.isEmpty(profileInfo) && (
                    <Forms
                        onSubmit={ (values) => handleSubmit(values, schema.scimName, schema.name) }
                    >
                        <Grid>
                            {
                                profileSchema && profileSchema.map((schema: ProfileSchema, index: number) => {
                                    if (schema.name !== "ibm-allGroups") {
                                        return (
            ...
            <List divided={ true } verticalAlign="middle" className="main-content-inner">
                {
                    profileSchema && profileSchema.map((schema: ProfileSchema, index: number) => {
                        if (schema.name !== "ibm-allGroups" && schema.name !== "userPassword" && schema.name !== "uid") {
                            return (
                                <List.Item key={ index } className="inner-list-item">
                                    { generateProfileEditForm(schema) }
                                </List.Item>
                            );
                        }
                    })
