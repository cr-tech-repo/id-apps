...
import { Button, EmptyPlaceholder, PrimaryButton } from "@cicis/react-components";
...
export const UsersPage: React.FunctionComponent<any> = (): ReactElement => {
    ...
    const getList = (limit: number, offset: number, filter: string) => {
        getUsersList(limit, offset, filter, "meta.lastModified,userName,id,name")
            .then((response) => {
                setUsersList(response);
            });
    };

    const getRolesList = (limit: number, offset: number, filter: string) => {
        getGroupsList(limit,offset,filter)
            .then((response) => {
                setRolesList(response.data.Resources);
            });
    };

    const getRoleListForDomain = () => {
        getGroupsList(undefined,undefined,undefined)
            .then((response) => {
    ...
    useEffect(() => {
        getRolesList(undefined,undefined,undefined);
    }, []);
